...
集成测试依赖宿主机访问 `127.0.0.1:8080`（或 `SERVER_HOST_PORT`）上的 API，MySQL/Redis 容器名默认 `kaizao-mysql` / `kaizao-redis`（与 compose 一致）。
...
新增接口覆盖说明:
  ...
  - **§团队实体对齐**（迁移 012）：**§5.3b** `GET /market/experts` 团队维度字段校验（`team_name`/`vibe_level`/`vibe_power`/`member_count`）；**§4.1c** `GET /home/demander` 推荐专家团队字段；**§2.2f** `PUT /users/me` 写入 `hourly_rate`/`available_status` 后 `GET /users/me` 回读一致。
"""
import json
