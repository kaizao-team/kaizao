...
> `🖥️ 前端` — **发布需求页 (PostPage)** 需求收集完毕，用户点击「确认」
...
{
  "project_id": "550e8400-...",
  "agent_message": "需求文档已确认，包含 12 条 EARS 需求规格",
  "sub_stage": "confirmed",
  "completeness_score": 100,
  "tool_name": "generate_ears_tasks",
  "document_path": "outputs/550e8400-.../v1/requirement.md"
}
```
...
> `🖥️ 前端` — **发布需求页 (PostPage)** 推荐使用流式版本，实时展示 AI 输出

| 接口 | 对应普通接口 | 请求体 |
|------|-------------|--------|
| `POST /api/v2/requirement/start/stream` | 同 3.1 | 同 3.1 |
| `POST /api/v2/requirement/{project_id}/message/stream` | 同 3.2 | 同 3.2 |
| `POST /api/v2/requirement/{project_id}/confirm/stream` | 同 3.3 | 同 3.3 |
...
| `POST /api/v2/requirement/{id}/confirm/stream` | 确认需求文档（推荐 SSE） |
...
③ POST /api/v2/requirement/{id}/message/stream  ← 用户补充信息
     ↓ （completeness_score 达到阈值）
④ POST /api/v2/requirement/{id}/confirm/stream  ← 确认需求文档
```
...
| 10 | POST | `/api/v2/requirement/{project_id}/confirm/stream` | SSE | 🖥️ 前端 | PostPage |
