...
5. [项目管理 PM](#5-项目管理-pm)
6. [生命周期 Lifecycle](#6-生命周期-lifecycle-go-后端调用)
7. [智能撮合 Match](#7-智能撮合-match)
8. [对话助手 Chat](#8-对话助手-chat)
9. [团队评级 Rating](#9-团队评级-rating)
10. [前端页面 → 接口映射总表](#10-前端页面--接口映射总表)
11. [前端调用时序](#11-前端调用时序)
...
> `🖥️ 前端` — **项目概览页 (ProjectOverviewPage)** / **项目卡片** PRD 确认后展示项目级摘要 + 需求条目
>
> 返回项目摘要、目标用户、复杂度、技术要求、非功能性需求，以及完整的 PRD 条目列表。一个接口满足项目概览页所需的全部数据。

```
GET /api/v2/pipeline/{project_id}/overview
```
...
```json
{
  "project_id": "550e8400-...",
  "title": "在线教育平台",
  "summary": "面向 K12 学生的在线直播+录播课程平台，支持多端访问",
  "target_users": [
    {"role": "学生", "description": "K12 阶段的学习者"},
    {"role": "教师", "description": "在线授课的讲师"}
  ],
  "complexity": "L",
  "tech_requirements": {
    "platform": "Web + App",
    "tech_stack": "Flutter + Go",
    "third_party": ["直播SDK", "支付"]
  },
  "non_functional_requirements": {
    "performance": "支持 500 人同时在线",
    "security": "用户数据加密存储"
  },
  "module_count": 5,
  "item_count": 18,
  ...
  "prd_items": [
    {
      ...
      "module_name": "用户系统",
      "title": "手机号注册登录",
      "description": "支持手机号+验证码注册和登录",
      "priority": "P0",
      "acceptance_summary": "用户能通过手机号完成注册和登录流程",
      "version": 1,
      "created_at": "2026-04-03T10:00:00"
    }
  ]
...
| `title` | 项目标题 |
| `summary` | 项目一句话摘要 |
| `target_users` | 目标用户画像列表 |
| `complexity` | 项目复杂度：S(1-3天) / M(3-7天) / L(7-15天) / XL(15-30天) |
| `tech_requirements` | 技术要求（平台、技术栈、第三方集成等） |
| `non_functional_requirements` | 非功能性需求（性能、安全、可用性等） |
| `module_count` | 功能模块数量 |
| `item_count` | 需求条目总数 |
| `prd_items` | 完整的 PRD 需求条目列表（同 2.5 接口返回内容） |

> 如果 PRD 尚未生成，返回 code=40402。

---

### 2.5 查询 PRD 需求条目

> `🖥️ 前端` — **项目概览页 (ProjectOverviewPage)** / **PRD文档页 (PrdPage)** PRD 确认后展示结构化需求条目

```
GET /api/v2/pipeline/{project_id}/prd-items
```
...
| 字段 | 说明 |
|------|------|
| `item_id` | 需求条目编号（如 F-1.1），按模块分组 |
| `module_name` | 所属功能模块名称 |
| `priority` | 优先级：P0（必须）/ P1（重要）/ P2（一般） |
| `acceptance_summary` | 用户视角的验收标准（非技术语言） |

> 前端可按 `module_name` 分组渲染模块树 + 需求卡片，配合 `priority` 做颜色标记。

---

### 2.6 查询里程碑列表

> `🖥️ 前端` — **项目管理页 (ProjectManagePage)** 撮合成功后展示里程碑时间线

```
GET /api/v2/pipeline/{project_id}/milestones
```
...
**触发行为：** 自动加载前序文档（requirement.md + design.md）+ 商定价格/工期 → PM Agent 重新生成项目管理方案 → 返回结构化里程碑
...
| 接口 | Content-Type | 说明 |
|------|-------------|------|
| `POST /api/v2/rating/evaluate/stream/file` | multipart/form-data | 文件上传 + SSE 流式，字段同 9.1 |
| `POST /api/v2/rating/evaluate/stream/text` | application/json | 纯文本 + SSE 流式，请求体同 9.2 |
...
> 需求方发起项目的核心页面，包含 AI 对话、需求采集全流程。EARS 完成后 design + PM 自动生成。
...
| `POST /api/v2/design/{id}/start/stream` | 手动触发架构设计（推荐 SSE，通常由 EARS 自动触发） |
| `POST /api/v2/design/{id}/confirm` | 确认设计方案 |

### 🖥️ 项目概览页 (ProjectOverviewPage)

> PRD 确认后展示项目摘要 + 结构化需求概览（模块树 + 需求卡片），供需求方和专家对齐需求

| 接口 | 用途 |
|------|------|
| `GET /api/v2/pipeline/{id}/overview` | **核心接口** — 一次拿到项目摘要 + 全部需求条目 |
| `GET /api/v2/pipeline/{id}/status` | 查询流水线进度和 sub_stage |
| `GET /api/v2/pipeline/{id}/documents` | 获取所有已生成文档列表 |
| `GET /api/v2/requirement/{id}/document` | 查看完整需求文档（可选） |

### 🖥️ PRD 文档页 (PrdPage)

> 查看已生成的 AI 文档（需求/设计），展示 EARS 规格和模块树

| 接口 | 用途 |
|------|------|
| `GET /api/v2/pipeline/{id}/status` | 查询流水线进度 |
| `GET /api/v2/pipeline/{id}/documents` | 获取所有文档列表 |
| `GET /api/v2/pipeline/{id}/prd-items` | 获取 PRD 需求条目（结构化展示） |
...
| `GET /api/v2/pipeline/{id}/prd-items` | 获取 PRD 需求条目 |
...
| `GET /api/v2/pipeline/{id}/milestones` | 获取结构化里程碑列表 |
...
### Phase 2: AI 文档自动生成（EARS 完成后自动触发）

```
EARS tasks_ready → 自动并行生成 design.md + project-plan.md
     ↓ （前端通过 GET /pipeline/{id}/status 轮询 design/pm 状态）
⑤ GET /api/v2/pipeline/{id}/status             ← 轮询 design + pm 状态
⑥ GET /api/v2/design/{id}/document             ← 查看自动生成的架构设计
⑦ GET /api/v2/pm/{id}/document                 ← 查看自动生成的初版 PM 方案
```
...
```
GET  /api/v2/pipeline/{id}/status              ← 🖥️ 项目详情/PRD页/概览页 查询进度
GET  /api/v2/pipeline/{id}/documents           ← 🖥️ 项目详情/PRD页/概览页 查询文档
GET  /api/v2/pipeline/{id}/overview            ← 🖥️ 概览页/项目详情页 获取项目摘要+需求条目
GET  /api/v2/pipeline/{id}/prd-items           ← 🖥️ PRD页/项目详情页 获取需求条目
GET  /api/v2/pipeline/{id}/milestones          ← 🖥️ 项目管理页 获取里程碑列表
GET  /api/v2/requirement/{id}/document         ← 🖥️ PRD页/概览页 获取需求文档
...
| 2 | GET | `/api/v2/pipeline/{project_id}/status` | JSON | 🖥️ 前端 | ProjectDetailPage / PrdPage / OverviewPage |
| 3 | GET | `/api/v2/pipeline/{project_id}/documents` | JSON | 🖥️ 前端 | ProjectDetailPage / PrdPage / OverviewPage |
| 3b | GET | `/api/v2/pipeline/{project_id}/overview` | JSON | 🖥️ 前端 | OverviewPage / ProjectDetailPage |
| 3c | GET | `/api/v2/pipeline/{project_id}/prd-items` | JSON | 🖥️ 前端 | PrdPage / ProjectDetailPage |
| 3d | GET | `/api/v2/pipeline/{project_id}/milestones` | JSON | 🖥️ 前端 | ProjectManagePage |
...
| 15 | GET | `/api/v2/pm/{project_id}/document` | JSON | 🖥️ 前端 | ProjectManagePage |
| 16 | POST | `/api/v2/pm/{project_id}/regenerate` | JSON | 🖥️ 前端 | ProjectManagePage |
| 17 | POST | `/api/v2/lifecycle/on-matched` | JSON | 🔧 Go后端 | — |
| 18 | POST | `/api/v2/lifecycle/on-started` | JSON | 🔧 Go后端 | — |
| 19 | POST | `/api/v2/lifecycle/on-milestone-delivered` | JSON | 🔧 Go后端 | — |
| 20 | POST | `/api/v2/lifecycle/on-completed` | JSON | 🔧 Go后端 | — |
| 21 | POST | `/api/v2/match/recommend` | JSON | 🖥️ 前端 | ProjectDetailPage |
| 22 | POST | `/api/v2/chat/message` | JSON | 🖥️ 前端 | ChatDetailPage |
| 23 | POST | `/api/v2/rating/evaluate/file` | multipart | 🖥️ 前端 | ExpertSupplementPage |
| 24 | POST | `/api/v2/rating/evaluate/text` | JSON | 🖥️ 前端 | ExpertSupplementPage |
| 25 | POST | `/api/v2/rating/evaluate/stream/file` | SSE | 🖥️ 前端 | ExpertSupplementPage |
| 26 | POST | `/api/v2/rating/evaluate/stream/text` | SSE | 🖥️ 前端 | ExpertSupplementPage |
| 27 | GET | `/api/v2/rating/{provider_id}/profile` | JSON | 🖥️ 前端 | ExpertLevelPage / ProfilePage |
| 28 | GET | `/api/v2/rating/{provider_id}/history` | JSON | 🖥️ 前端 | ProfilePage |
| 29 | POST | `/api/v2/rating/{provider_id}/adjust` | JSON | 🔑 管理端 | — |
