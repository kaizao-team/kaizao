...
所有界面实现与改版，必须在提交前完成以下检查：

- 禁止出现 `RenderFlex overflowed`
- 禁止出现文本被裁切、徽标被顶出、按钮文案被压坏这类边界问题
- 默认按 `compact` 断点和常规主流手机尺寸都跑一遍，确认不会白屏、不会错位、不会出现不可点击区域
- 所有 `Row` / `Column` / `Wrap` 组合都要主动考虑长文本、极短文本和选中态切换，必要时使用 `Expanded`、`Flexible`、`Wrap`、`maxLines`、`TextOverflow.ellipsis`
- 能用自然高度布局就不要用固定高度；能避免绝对定位叠层就避免，防止动画和小屏条件下出现约束冲突
- UI 改完后，必须实际启动页面检查，而不是只看代码或只跑 analyze
