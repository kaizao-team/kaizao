...
class _ExpertProfilePageState extends ConsumerState<ExpertProfilePage> {
  ...
  final Set<String> _serviceDirections = {};
  String? _avatarUrl;
  ...
  Future<void> _next() async {
    ...
    // Save service directions to draft
    if (_serviceDirections.isNotEmpty) {
      await notifier
          .saveDraft({'service_directions': _serviceDirections.toList()});
    }
    final success = await notifier.submitExpertProfile(
      ...
      serviceDirections: _serviceDirections.toList(),
    );
    if (!mounted) return;
          ...
          OnboardingDeckCard(
            child: Wrap(
              ...
              children: projectCategorySpecs.map((spec) {
                final isSelected = _serviceDirections.contains(spec.key);
                return OnboardingChip(
                  ...
                  onTap: () {
                    setState(() {
                      if (isSelected) {
                        _serviceDirections.remove(spec.key);
                      } else {
                        _serviceDirections.add(spec.key);
                      }
                    });
                  },
                );
...
class _ExpertProfilePreviewCard extends StatelessWidget {
  ...
  final List<String> serviceDirections;
    ...
    return OnboardingDeckCard(
      ...
      child: Column(
        ...
        children: [
          ...
          if (serviceDirections.isNotEmpty) ...[
            const SizedBox(height: 8),
            Wrap(
              spacing: 6,
              runSpacing: 4,
